...
6. **Shared accent `#474e91`** (Jellybeans' selection indigo; succeeded the
   tmux-heritage `#00afff` when the theme moved to Jellybeans, 2026-08)
   appears in FOUR places that must change together: JankyBorders
   `active_color` in aerospace/aerospace.toml, `ui.accent` in
   ...
   layer. The accent must be a SELECTION-grade color: herdr paints the
   theme's whitish text over it in active sidebar rows (no text-on-accent
   override exists — verified against theme.custom via reload diagnostics),
   so a light accent like the theme's bright blue makes rows unreadable.
   Light-on-accent counterparts live in tmux (`window_status_current_fg` =
   colour_7) and i3 (`client.focused` text). Same coupling class: herdr's
   theme (`name = "terminal"`) inherits Ghostty's `theme` — a theme change
   re-derives the accent from the new theme's SELECTION color, not its
   brightest hue. JankyBorders `inactive_color` (#303030) re-derives too.
